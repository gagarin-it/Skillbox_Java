...
public class Main {
  ...
  public static void main(String[] args) {
    try (Connection connection = DriverManager.getConnection(URL_MYSQL, USER_MYSQL, PASSWORD_MYSQL)){
      ...
      ResultSet resultSet = statement.executeQuery("SELECT pl.course_name, (count(month(pl.subscription_date))/"
          + "max(month(pl.subscription_date))) as average_of_count_buy FROM PurchaseList pl "
          + "group by pl.course_name order by pl.course_name");
      int courseId = 0;
      System.out.printf("     %-35s%-16s%n", "Наименование курса", "Среднее количество покупок");
